...
exports.generateChaoticPath = generateChaoticPath;
function pickCrashMultiplier() {
    const r = Math.random();
    if (r < 0.49)
        return 2; // 49% of games end below 2x
    if (r < 0.57)
        return 10; // 8% of games below 10x
    if (r < 0.59)
        return 50; // 2% below 50x
    return 100; // 1% below 100x
}
function randomRugValue() {
    return parseFloat((Math.random() * 0.000099 + 0.000001).toFixed(6));
}
function generateChaoticPath() {
...
//# sourceMappingURL=price_ticks.js.map